...
void EXPORT_API VRPNServerStart (const char* config_file_name, const char* location, bool verbose=true) 
{
	#if _MSC_VER
...
void EXPORT_API VRPNServerLoop () 
{
...
void EXPORT_API VRPNServerStop () 
{ 
...
void EXPORT_API VRPNTrackerStart(const char* name, int type=VRPN_POS, int max=1000) 
{ 
	if (connection == NULL) {
...
void EXPORT_API VRPNTrackerPosReport(const char* name, vrpn_TRACKERCB* cpy, timeval* ts, int sensor=0)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr)
...
void EXPORT_API VRPNTrackerVelReport(const char* name, vrpn_TRACKERVELCB* cpy, timeval* ts, int sensor=0)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr)
...
void EXPORT_API VRPNTrackerAccReport(const char* name, vrpn_TRACKERACCCB* cpy, timeval* ts, int sensor=0)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr)
...
int EXPORT_API VRPNTrackerNumPosReports(const char* name)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr)
...
int EXPORT_API VRPNTrackerNumVelReports(const char* name)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr)
...
int EXPORT_API VRPNTrackerNumAccReports(const char* name)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr)
...
void EXPORT_API VRPNTrackerPosReports(const char* name, vrpn_TRACKERCB** cpy,int& cnt)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr != NULL)
...
void EXPORT_API VRPNTrackerVelReports(const char* name, vrpn_TRACKERVELCB** cpy,int& cnt)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr)
...
void EXPORT_API VRPNTrackerAccReports(const char* name, vrpn_TRACKERACCCB** cpy,int& cnt)
{
	std::string device(name);
	VRPNTrackerInstance* tkr = trackers[device];
	if (tkr)
...
void EXPORT_API VRPNAnalogStart(const char* name, int max=1000) 
{ 
	if (connection == NULL) {
...
int EXPORT_API VRPNAnalogNumReports(const char* name)
{
	std::string device(name);
	VRPNAnalogInstance* ana = analogs[device];
	if (ana)
...
void EXPORT_API VRPNAnalogReports(const char* name, vrpn_ANALOGCB** cpy, int& cnt, timeval* ts, bool clearReport)
{
	std::string device(name);
	VRPNAnalogInstance* ana = analogs[device];
	if (ana)
...
void EXPORT_API VRPNButtonStart(const char* name, int max=1000) 
{ 
	if (connection == NULL) {
...
int EXPORT_API VRPNButtonNumReports(const char* name)
{
	std::string device(name);
	VRPNButtonInstance* btn = buttons[device];
	if (btn)
...
void EXPORT_API VRPNButtonReports(const char* name, vrpn_BUTTONCB** cpy, int& cnt, timeval* ts, int btn_num, bool clearReport)
{
	std::string device(name);
	VRPNButtonInstance* btn = buttons[device];
	if (btn)
...
bool EXPORT_API VRPNTimevalGreater(timeval* tv1, timeval* tv2)
{
